	...
	socket.on('updateDisplay', function(data) {	
      ...
      //The abreviations here may not all be correct, they are guesses.
        ...
        if(i<result.length-1){
          io.sockets.emit("updateResult",{user:data.userName, x:timeStringF, y:result[i].realP, final:0});
        }
        else{
          io.sockets.emit("updateResult",{user:data.userName, x:timeStringF, y:result[i].realP, final:1});
        }
